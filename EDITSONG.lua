...
local function EDITSONG()
	...
	if(type(sSeqCopy) ~= "number") or (type(sMakerCopy) ~= "number") or (type(sAdderCopy) ~= "number") then
		gma.gui.msgbox("ERROR" , "EDITSONG Plugin was NOT setup!\nPlease read manual for more information on setting up plugin!")
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Plugin Error. EDITSONG is not setup.")
		gma.echo(ESC_WHT .."EDITSONG Plugin : " ..ESC_RED .."Plugin Error. EDITSONG is not setup.")
		return -13;
	end

	if(type(localPoolSize) ~= "number") then
		gma.gui.msgbox("ERROR" , "EDITSONG Plugin was NOT setup!\n\nPlease setup how many columns you want \nto use for your MAKER+ library!")
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Plugin Error. pool_size is not setup.")
		gma.echo(ESC_WHT .."EDITSONG Plugin : " ..ESC_RED .."Plugin Error. pool_size is not setup.")
		return -13;
	elseif(localPoolSize <= 1) then
		...
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Plugin Error. pool_size is too small of a number.")
		gma.echo(ESC_WHT .."EDITSONG Plugin : " ..ESC_RED .."Plugin Error. pool_size is too small of a number.")
		return -13;
	end
			...
			for index = sSeqCopy , eSeqCopy - 1 , 1 do
				if (0 ~= math.fmod(index , localPoolSize)) then
					if (G_OBJ.label(G_OBJ.handle("Macro " ..makerTarget)) == string.gsub(G_OBJ.label(G_OBJ.handle("Sequence " ..index)) , "_" , " ")) then
						seqTarget = index
						renameSong = G_OBJ.label(G_OBJ.handle("Macro " ..makerTarget))
					end
				end
			end
	...
	gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_YEL .."========================================")
	gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_YEL .."Song Name: " ..ESC_WHT ..gma.show.getobj.label(gma.show.getobj.handle("Macro " ..makerTarget)))
	gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_YEL .."Song Sequence: " ..ESC_WHT ..seqTarget)
	gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_YEL .."========================================")
	...
	--gma.cmd("Assign Sequence " ..availSeq .." At Executor " ..gma.show.getobj.number(gma.user.getselectedexec()))
	  gma.cmd("Assign Sequence " ..seqTarget .." At Executor " ..gma.show.getvar('SELECTEDEXEC'))
