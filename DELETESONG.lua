...
local function stringSongConv (poolOneName , poolOneStart , poolOneEnd , poolTwoName , poolTwoStart , poolTwoEnd , poolThreeName , poolThreeStart , poolThreeEnd , songName , poolSize)
	...
	-- Escape Sequences... Color commandline and system monitor
	...
	-- loop to find other version of the song ending with "(...)_V"
	-- Song Sequence Library
	...
	-- Is there a missing Maker or Adder Macro; If so, tell user to resolve situation and exit
	if (verCountOne ~= verCountTwo or verCountOne ~= verCountThree) then
		...
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Please Resolve: " ..songName .." versions")
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolOneName .."s have " ..verCountOne - 1 .." versions of " ..songName)
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolTwoName .."s for Maker have " ..verCountTwo - 1 .." versions of " ..songName)
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolThreeName .."s for Adder have " ..verCountThree - 1 .." versions of " ..songName)
		return -13 , -26 , -39;
	end

	-- Did not find a song matching the name of that song
	if (verCountOne == 1) then
		...
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..songName .." does not exist in the song library.")
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolOneName .."s have " ..verCountOne - 1 .." versions of " ..songName)
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolTwoName .."s for Maker have " ..verCountTwo - 1 .." versions of " ..songName)
		gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED ..poolThreeName .."s for Adder have " ..verCountThree - 1 .." versions of " ..songName)
		return -13 , -26 , -39;
		...
		repeat
			...
			if (boolContinue == false) then
				if (false == (gma.gui.confirm("ERROR" , "Invalid Song Name! \n Make sure to only put numbers"))) then
					gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Plugin Terminated. Song not assigned to selected executor.")
					gma.echo(ESC_WHT .."EDITSONG Plugin : " ..ESC_RED .."Plugin Terminated. Song not assigned to selected executor.")
					return -13 , -26 , -39;
				end
			end
			...
			if (userReq == nil) then
				...
				return -13 , -26 , -39;
			elseif (userReq == "") then
				-- The forgot to type a version number! Retry!
				boolContinue = false
			elseif (tonumber(userReq) == nil) then
				-- no words involved! Only numbers
				boolContinue = false
			elseif (tonumber(userReq) >= verCountOne) and (tonumber(userReq) <= 0) then
				-- They choose a version number out of range
				boolContinue = false
			end
...
local function condDelt(poolString , poolStart , poolSize)
	local G_OBJ = gma.show.getobj
	...
	for loopIndex = poolStart , countPool , 1 do
		if (0 == math.fmod(loopIndex , poolSize)) then
			-- skip the most left pool object of a 16 wide pool
			goto skipSixteen
		end
		if (not(G_OBJ.verify(G_OBJ.handle(poolString  .." " ..loopIndex)))) then
			while ((not(G_OBJ.verify(G_OBJ.handle(poolString .." " ..loopIndex + counterGap)))) and (loopIndex + counterGap <= countPool)) do
				counterGap = counterGap + 1
			end

			if (loopIndex + counterGap <= countPool) then
...
local function DELETESONG()
	...
	if(type(sSeqCopy) ~= "number") or (type(sMakerCopy) ~= "number") or (type(sAdderCopy) ~= "number") then
		gma.gui.msgbox("ERROR" , "DELETESONG Plugin was NOT setup!\nPlease read manual for more information on setting up plugin!")
		gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_RED .."Plugin Error. EDITSONG is not setup.")
		gma.echo(ESC_WHT .."DELETESONG Plugin : " ..ESC_RED .."Plugin Error. EDITSONG is not setup.")
		return -13;
	end

	if(type(localPoolSize) ~= "number") then
		gma.gui.msgbox("ERROR" , "DELETESONG Plugin was NOT setup!\n\nPlease setup how many columns you want \nto use for your MAKER+ library!")
		gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_RED .."Plugin Error. pool_size is not setup.")
		gma.echo(ESC_WHT .."DELETESONG Plugin : " ..ESC_RED .."Plugin Error. pool_size is not setup.")
		return -13;
	elseif(localPoolSize <= 1) then
		...
		gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_RED .."Plugin Error. pool_size is too small of a number.")
		gma.echo(ESC_WHT .."DELETESONG Plugin : " ..ESC_RED .."Plugin Error. pool_size is too small of a number.")
		return -13;
	end
	...
	if (tonumber(renameSong) == nil) then
		...
		if (G_OBJ.verify(G_OBJ.handle("Macro " ..makerTarget))) then
			...
			gma.feedback(ESC_WHT .."EDITSONG : " ..ESC_RED .."Macro " ..renameSong .." does not exist. Plugin terminated.")
			gma.echo(ESC_WHT .."EDITSONG Plugin : " ..ESC_RED .."Plugin terminated.")
			return -13;
		end
	end

	local songName = gma.show.getobj.label(gma.show.getobj.handle("Macro " ..makerTarget))

	gma.cmd("Delete Sequence " ..seqTarget .." /nc")
	gma.cmd("Delete Macro " ..makerTarget .." /nc")
	gma.cmd("Delete Macro " ..adderTarget .." /nc")

	if(gma.gui.confirm("Condense MAKER+ Libraries" , "Would you like to condense the MAKER+ libraries?")) then
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-START-" ..ESC_RED .."========================")
		condDelt("Sequence" , sSeqCopy , localPoolSize)
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-END-" ..ESC_RED .."==========================")
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-START-" ..ESC_RED .."========================")
		condDelt("Macro" , sMakerCopy , localPoolSize)
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-END-" ..ESC_RED .."==========================")
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-START-" ..ESC_RED .."========================")
		condDelt("Macro" , sAdderCopy , localPoolSize)
		gma.echo(ESC_RED .."========================" ..ESC_WHT .."-INTENTIONAL-SYNTAX-ERROR-END-" ..ESC_RED .."==========================")
	end

	-- print on command line feedback information about what and where the new song is
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."========================================")
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."Song  Name: " ..ESC_RED ..songName)
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."Song   SEQ: " ..ESC_GRN .."Sequence " ..ESC_WHT ..seqTarget)
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."Song MAKER: " ..ESC_GRN .."Macro " ..ESC_WHT ..makerTarget)
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."Song ADDER: " ..ESC_GRN .."Macro " ..ESC_WHT ..adderTarget)
	gma.feedback(ESC_WHT .."DELETESONG : " ..ESC_YEL .."========================================")
